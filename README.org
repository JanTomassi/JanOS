...
** Build
*** Tools
These tools are needed to bootstrap the cross toolchain and build the
kernel:

- bash
- make
- gcc
- nasm (system-wide, required by the build as `nasm -f elf32`)
- tar
- xz

Enter in the directory tools and execute the bash script get-tools.sh.
It builds the required i686-elf binutils/GCC/GDB toolchain into
`tools/build/bin` (the Makefile already points CC/AR there).
#+begin_src bash
  cd tools && ./get-tools.sh
#+end_src
This will download and compile all the tools needed for development,
except for nasm (install it via your package manager).
