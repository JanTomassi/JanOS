...
** Build
*** Tools
This tools are needed to compile the build environment. ~nasm~ is
required for assembling the low-level stubs. The cross toolchain
(~i686-elf-gcc~, ~i686-elf-ld~, etc.) is built by running
~tools/get_tools.sh~, which installs into ~tools/build/bin~.
...
Enter in the directory tools and execute the bash script ~get_tools.sh~
#+begin_src bash
  cd tools && ./get_tools.sh
#+end_src
this will download and compile the tools needed for development except
for ~nasm~, which should be installed separately.
