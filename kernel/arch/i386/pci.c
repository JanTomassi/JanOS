...
static bool match_class(uint8_t bus, uint8_t slot, uint8_t func,
...
bool pci_find_storage(uint8_t class_code, uint8_t subclass,
		      struct pci_device_info *out)
{
	if (!out)
		return false;
	...
	for (uint16_t bus = 0; bus < 256; bus++) {
		...
		for (uint8_t slot = 0; slot < 32; slot++) {
			...
			for (uint8_t func = 0; func < max_func; func++) {
				...
				if (!match_class(bus_id, slot, func, class_code,
						 subclass))
					continue;
