...
void idt_set_descriptor(uint8_t vector, void *isr, uint8_t flags)
{
	...
	descriptor->kernel_cs =
		0x08; // this value can be whatever offset your kernel code selector is in your GDT
...
void idt_init(void)
{
	...
	for (uint16_t vector = 0; vector < (32 + 16); vector++) {
		idt_set_descriptor(vector, (void *)isr_stub_table[vector],
				   PRESENT | DPL_KERNEL_LEVEL |
					   (vector < 20 ? GATE_TYPE_TRAP :
							  GATE_TYPE_INTERRUPT));
	}
