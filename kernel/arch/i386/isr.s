extern _GLOBAL_OFFSET_TABLE_ extern kprintf

	section.rodata isr_default_msg : db "Default interrupt: %u",
	0xA,
	0

		section.data global isr_stub_table isr_stub_table : %
								    assign i 0 % rep 256 dd isr_stub_ % +i;
use DQ instead if targeting 64 - bit % assign i i +
	1 %
		endrep
	...
	_GLOBAL_OFFSET_TABLE_ + $$ - % % getgot wrt..gotpc
					     ...
					     % macro isr_err_stub 1 extern isr_ % +%
					     1_handler : function
							 ...
							 1_handler :;
;
printing the default msg push 0 push % 1 push isr_default_msg call kprintf add esp,
	12
	...
	eax pop eax
		...
		% macro isr_no_err_stub 1 extern isr_ % +%
		1_handler : function
			    ...
			    1_handler :;
;
printing the default msg push 0 push % 1 push isr_default_msg call kprintf add esp, 12

											    ret

												    global isr_stub_ %
											    +% 1 : function isr_stub_ % +%
												   1 : pusha
												       ...
												       %
												       macro isr_irq_master_stub 1 global isr_ %
												       +%
												       1_handler : function weak extern isr_ %
														   +%
														   1_handler : function strong
...
acknowledge the interrupt to the PIC
	...
	% macro isr_irq_slave_stub 1 global isr_ % +% 1_handler : function weak extern isr_ % +%
								  1_handler : function strong
