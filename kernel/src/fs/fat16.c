...
static bool fat16_read_root_dir_sectors(const struct block_device *device, const fat16_layout_t *layout,
					uint8_t *out)
{
	if (layout->root_dir_sectors == 0 || layout->root_dir_sectors > UINT16_MAX)
		return false;

	return block_device_read(device, layout->root_dir_lba,
				   (uint16_t)layout->root_dir_sectors, out);
}

static bool fat16_read_sectors(const struct block_device *device, uint32_t lba,
			       uint16_t count, void *out)
{
	return block_device_read(device, lba, count, out);
}
...
static bool fat16_load_layout(const struct block_device *device, fat16_layout_t *layout,
			      fat_BS_t **out_bpb)
{
...
static bool fat16_read_fat_sector(const struct block_device *device, const fat16_layout_t *layout,
...
static bool fat16_read_cluster_data(const struct block_device *device, const fat16_layout_t *layout,
