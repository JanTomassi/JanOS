...
mem_malloc_tag_t *mem_coalesce_tag(mem_malloc_tag_t *tag)
{
	...
	if (coalesce_next) {
		tag->size += next->size;
		for (struct list_head *it = next->phy_chain.next; it != &next->phy_chain;) {
			struct list_head *next_it = it->next;
			...
			struct list_head *elm = &phy_link->list;
			list_rm(elm);
...
continue_next:
	...
	if (coalesce_prev) {
		prev->size += tag->size;
		for (struct list_head *it = tag->phy_chain.next; it != &tag->phy_chain;) {
			struct list_head *next_it = it->next;
			mem_phy_mem_link_t *phy_link = list_entry(it, mem_phy_mem_link_t, list);
			mem_phy_mem_tag_t *phy_tag = phy_link->phy_mem;
			...
			struct list_head *elm = &phy_link->list;
			list_rm(elm);
			list_add(elm, prev->phy_chain.prev);
continue_curr:
			it = next_it;
		}
	}
