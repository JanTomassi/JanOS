...
static bool ahci_port_rebase(struct ahci_port_state *state)
{
	...
	/* This driver serializes requests, so one command slot is sufficient. */
	for (int i = 0; i < 1; i++) {
...
bool ahci_probe(void)
{
	...
	mprint("AHCI controller %u:%u.%u BAR5=%x IRQ=%u\n", dev.bus, dev.device,
		dev.function, dev.bar[5], dev.irq_line);
	return true;
}
...
void ahci_init(void)
{
	...
	if (bar5 == 0) {
		mprint("AHCI controller has no MMIO BAR\n");
		return;
	}
	...
	if (ahci_state.mmio.virt == nullptr) {
		mprint("AHCI MMIO mapping failed\n");
		return;
	}
	...
	mprint("AHCI implemented ports=%x\n", implemented);
	for (uint8_t i = 0; i < AHCI_MAX_PORTS; i++) {
