...
int main(void)
{
	...
	TEST_ASSERT(display_session_claim(&session, 7));
	...
	TEST_ASSERT(!display_session_allows(&session, 0));
	TEST_ASSERT(display_session_claim(&session, 7));
	TEST_ASSERT(!display_session_claim(&session, 8));
	TEST_ASSERT(!display_session_release(&session, 8));
	TEST_ASSERT(display_session_release(&session, 7));
	TEST_ASSERT(session.mode == DISPLAY_SESSION_BOOT);
	TEST_ASSERT(session.owner == 0);
	return 0;
}
